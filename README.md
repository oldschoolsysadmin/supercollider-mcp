...
For an AI working through this MCP, these tools are the equivalent of tab-completion and hover documentation in an IDE — and for SuperCollider specifically, they are closer to a hard requirement than a nice-to-have. SC is niche enough that most models, including large ones, have sparse and unreliable training coverage of its API; smaller models that run on consumer hardware will confidently produce plausible-looking but broken code. By grounding every code generation step in the actual `.schelp` files from the installed version, the model can look up exact argument names, default values, and rates for any class — including quarks — rather than guessing. The result is meaningfully more accurate patch construction from the first attempt, without the back-and-forth of correcting hallucinated method signatures.
